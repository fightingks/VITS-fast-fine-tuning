...
def run_cpu(rank, n_gpus, hps):
    ...
    train_loader = DataLoader(train_dataset, batch_size=hps.train.batch_size, num_workers=2, shuffle=True, pin_memory=False, collate_fn=collate_fn)

    if rank == 0:
        eval_dataset = TextAudioSpeakerLoader(hps.data.validation_files, hps.data, symbols)
        eval_loader = DataLoader(eval_dataset, batch_size=hps.train.batch_size, num_workers=0, shuffle=False, pin_memory=False, drop_last=False, collate_fn=collate_fn)
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        if rank == 0:
            train_and_evaluate_cpu(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, eval_loader], logger, [writer, writer_eval])
        else:
            train_and_evaluate_cpu(rank, epoch, hps, [net_g, net_d], [optim_g, optim_d], [scheduler_g, scheduler_d], scaler, [train_loader, None], None, None)
        scheduler_g.step()
        scheduler_d.step()
...
def train_and_evaluate_cpu(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
    net_g, net_d = nets
    optim_g, optim_d = optims
    scheduler_g, scheduler_d = schedulers
    train_loader, eval_loader = loaders
    ...
    for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader)):
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                evaluate_cpu(hps, net_g, eval_loader, writer_eval)
...
def evaluate_cpu(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
        for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(eval_loader):
            # 将数据移动到CPU
